...
export default function ExampleCommands() {
  return <Box sx={{marginLeft: '5px', padding: '1em',}}>
    ...
    <Typography>
      <Command command={`tmsh -c 'cd /;show ltm virtual recursive field-fmt all'`}/> gets
      the virtual server stats from all partitions.</Typography>
    <Typography sx={{marginBottom: '1em'}}>
      <Command command={`tmsh -c 'cd /;show ltm pool recursive field-fmt all'`}/> gets
      the pool server stats from all partitions.
    </Typography>
    <Typography>
      Yeah, you get it, right? Just <i>always</i> use the <Code text={'field-fmt'}/> and
      add <Code text={'cd /;'}/> and <Code text={'recursive'}/> and any show command should work.
    </Typography>
    <Typography mt={2}>
